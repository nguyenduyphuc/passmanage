...
class Schema extends DB_Utils
{
    ...
    public function renameTable($name, $new_name, $exec = true)
    {
        ...
        if (preg_match('/odbc/', $this->db->driver())) {
            ...
            $cmd = array(
                'sqlite2?|pgsql' => "ALTER TABLE $name RENAME TO $new_name;",
                'mysql|ibm' => "RENAME TABLE $name TO $new_name;",
                'mssql|sqlsrv|sybase|dblib|odbc' => "sp_rename {$name}, $new_name",
            );
            $query = $this->findQuery($cmd);
            if (!$exec) {
                return $query;
    ...
    public function dropTable($name, $exec = true)
    {
        ...
        $cmd = array(
            'mysql|ibm|sqlite2?|pgsql|sybase|dblib' => 'DROP TABLE IF EXISTS '.$this->db->quotekey($name).';',
            'mssql|sqlsrv|odbc' => "IF OBJECT_ID('[$name]', 'U') IS NOT NULL DROP TABLE [$name];",
        );
        $query = $this->findQuery($cmd);
...
abstract class TableBuilder extends DB_Utils
{
    ...
    protected function _addIndex($index_cols, $search_cols, $unique, $length)
    {
        ...
        $cmd = array(
            'pgsql|sqlite2?|ibm|mssql|sybase|dblib|odbc|sqlsrv' => "CREATE $index $name ON $table ($cols);",
    ...
    public function primary($pkeys)
    {
        ...
        if (count($pkeys) > 1) {
            ...
            if (preg_match('/sqlite2?/', $this->db->driver())) {
                ...
                $cmd = array(
                    'odbc' => "CREATE INDEX $table_key ON $table ( $pk_string );",
                    'mysql' => "ALTER TABLE $table DROP PRIMARY KEY, ADD PRIMARY KEY ( $pk_string );",
...
class TableCreator extends TableBuilder
{
    ...
    public function build($exec = true)
    {
        ...
        $cmd = array(
            'sqlite2?|sybase|dblib' => "CREATE TABLE $table ($id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT".$cols.');',
            'mysql' => "CREATE TABLE $table ($id INTEGER NOT NULL PRIMARY KEY AUTO_INCREMENT".$cols.') DEFAULT CHARSET=utf8 COLLATE utf8_unicode_ci;',
            'pgsql' => "CREATE TABLE $table ($id SERIAL PRIMARY KEY".$cols.');',
            'mssql|odbc|sqlsrv' => "CREATE TABLE $table ($id INT IDENTITY CONSTRAINT PK_".$this->name.'_ID PRIMARY KEY'.$cols.');',
            'ibm' => "CREATE TABLE $table ($id INTEGER AS IDENTITY NOT NULL $cols, PRIMARY KEY($id));",
        );
        $query = $this->findQuery($cmd);
...
class TableModifier extends TableBuilder
{
    ...
    public function build($exec = true)
    {
        ...
        foreach ($this->columns as $cname => $column) {
            ...
            if ($sqlite) {
                ...
                $cmd = array(
                    'mysql|pgsql|mssql|sybase|dblib|odbc|sqlsrv' => "ALTER TABLE $table ADD $col_query;",
                    'ibm' => "ALTER TABLE $table ADD COLUMN $col_query;",
                );
                $this->queries[] = $this->findQuery($cmd);
            }
        }
        if ($sqlite) {
            if ($rebuild || !empty($this->rebuild_cmd)) {
    ...
    public function dropColumn($name)
    {
        ...
        if (preg_match('/sqlite2?/', $this->db->driver())) {
            ...
            $cmd = array(
                ...
                'pgsql|odbc|ibm|mssql|sybase|dblib|sqlsrv' => "ALTER TABLE $quotedTable DROP COLUMN $quotedColumn;",
            );
    ...
    public function renameColumn($name, $new_name)
    {
        ...
        if (preg_match('/sqlite2?/', $this->db->driver())) {
            ...
            $cmd = array(
                'mysql' => "ALTER TABLE $quotedTable CHANGE $quotedColumn $quotedColumnNew ".$existing_columns[$name]['type'].';',
                'pgsql|ibm' => "ALTER TABLE $quotedTable RENAME COLUMN $quotedColumn TO $quotedColumnNew;",
                'mssql|sybase|dblib|sqlsrv' => "sp_rename [$this->name.$name], '$new_name', 'Column'",
            );
            if (preg_match('/mssql|sybase|dblib|sqlsrv/', $this->db->driver())) {
                $this->suppress = true;
            }
    ...
    public function updateColumn($name, $datatype, $force = false)
    {
        ...
        if (preg_match('/sqlite2?/', $this->db->driver())) {
            ...
            $cmd = array(
            'mysql' => "ALTER TABLE $table MODIFY COLUMN $column $datatype;",
            'pgsql' => "ALTER TABLE $table ALTER COLUMN $column TYPE $datatype;",
            'sqlsrv|mssql|sybase|dblib|ibm' => "ALTER TABLE $table ALTER COLUMN $column $datatype;",
            );
            $this->queries[] = $this->findQuery($cmd);
        }
    }
    ...
    public function dropIndex($name)
    {
        ...
        $table = $this->db->quotekey($this->name);
        $cmd = array(
            'pgsql|sqlite2?|ibm' => "DROP INDEX $name;",
            'mssql|sybase|dblib|odbc|sqlsrv' => "DROP INDEX $table.$name;",
    ...
    public function listIndex()
    {
        $table = $this->db->quotekey($this->name);
        $cmd = array(
            'sqlite2?' => "PRAGMA index_list($table);",
            'mysql' => "SHOW INDEX FROM $table;",
            'mssql|sybase|dblib|sqlsrv' => 'select * from sys.indexes '.
