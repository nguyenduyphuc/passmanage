...
{
    // config
    // DB object [ \DB\SQL, \DB\Jig, \DB\Mongo ]
    protected $db;
    // selected table, string
    protected $table;
    // fluid sql schema mode, boolean
    protected $fluid;
    // field configuration, array
    protected $fieldConf;
    // default mapper schema ttl
    protected $ttl;
    // default mapper rel ttl
    protected $rel_ttl;
    // SQL table primary key behaviour
    protected $primary;
    // intelligent lazy eager loading, boolean
    protected $smartLoading;
    // return standardized '_id' field for SQL when casting internals
    protected $standardiseID;
    // mapper engine type [jig, sql, mongo]
    protected $dbsType;
    ...
    // filter for loading related models
    protected $relFilter;
    // IDs of records the next find should have
    protected $hasCond;
    // restrict to these fields
    protected $whitelist;
    // restrict relations to these fields
    protected $relWhitelist;
    // stack of group conditions
    protected $grp_stack;
    // relational counter buffer
    protected $countFields;
    // bind values to be prepended to $filter
    protected $preBinds;
    // virtual fields buffer
    protected $vFields;
    // rel_ttl overwrite
    protected $_ttl;         
    ...
    public static $init = false;  // just init without mapper
     ...
     * @param array $config
     */
    public function setFieldConfiguration(array $config)
    {
     ...
     * @return string
     */
    protected static function getMMTableName($ftable, $fkey, $ptable, $pkey, $fConf = null)
    {
        if ($fConf) {
        ...
        if (is_object($val)) {
            ...
            foreach ($val as &$item) {
                if (is_object($item) &&
                    ...
                    if (!$item instanceof self || $item->dry()) {
                        trigger_error(self::E_INVALID_RELATION_OBJECT);
                    } else {
                        $item = $item->get($rel_field, true);
                    }
                }
    ...
    protected function getRelInstance($model = null, $relConf = null, $key = '', $pushFilter = false)
    {
        ...
        if (\Registry::exists($relName)) {
            ...
            $rel = $model ? new $model() : new self($relConf['db'], $relConf['table']);
     ...
     */
    public function castField($key, $rel_depths = 0)
    {
        if (!$key) {
            return;
        }
     ...
     * @return bool
     */
    public function exists($key, $relField = false)
    {
        if (!$this->dry() && $key == '_id') {
            return true;
        }
...
class CortexCollection extends \ArrayIterator
{
     ...
     * @param $models
     */
    public function setModels($models, $init = true)
    {
     ...
     * @param $model
     */
    public function add(Cortex $model)
    {
