...
final class Base extends Prefab implements ArrayAccess
{
     ...
     **/
    public function alias($name, $params = array())
    {
        if (!is_array($params)) {
     ...
     *   @param $add bool
     **/
    public function &ref($key, $add = true)
    {
        $null = null;
        $parts = $this->cut($key);
        if ($parts[0] == 'SESSION') {
     ...
     **/
    public function reroute($url = null, $permanent = false)
    {
        if (!$url) {
            $url = $this->hive['REALM'];
        }
...
public function until($func, $args = null, $timeout = 60)
{
     ...
     *   @param $allow bool
     **/
    public function config($file, $allow = false)
    {
        preg_match_all(
            '/(?<=^|\n)(?:'.
                '\[(?<section>.+?)\]|'.
...
class Preview extends View
{
     ...
     *   @param $hive array
     **/
    public function resolve($str, array $hive = null)
    {
        if (!$hive) {
