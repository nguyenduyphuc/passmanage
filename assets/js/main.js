...
jQuery(function($){
  ...
  if( PassHub.mode == 'logins' ) {
    ...
    getItemJSON('categorieswithprivate', PassHub.csrf, function(data) { 
      if( data === false ) {
        ...
        Materialize.toast('Error: no categories found', 4000);
      } else {
        // success
    ...
    $('body').on('click', '.loginSingle .btnSave', function(e) {
      ...
      $('.input-field', loginEl).each(function() {
        ...
        fieldData.push({
          ...
          fieldType: fieldType,
          fieldName: slugify($('label', this).text()),
      ...
      // returns loginId
      var jqxhr = $.post(baseUrl + '/logins/save', {loginData: loginData, fieldData: fieldData, csrf: PassHub.csrf}, function(data) { 
        //console.log('save OK');
        // Set login-id on element in case it's a new login that doesn't have ID filled in yet
        loginEl.attr('data-login-id', data);
        
        Materialize.toast('Login saved', 4000);
      })
        .always(function() {
        ...
        .fail(function(jqxhr) {
          ...
          if(jqxhr.responseText != '') {
            ...
            error = 'Error: failed to update login';
          }
        ...
        var jqxhr2 = $.post(
          ...
          function() {})
            ...
            .fail(function(jqxhr2) {
              Materialize.toast('Error: failed to delete fields', 4000);
            });
      }
  ...
  if( PassHub.mode == 'categories' ) {
    ...
    $('body').on('click', '.btnSave', function(e) {
        ...
        var jqxhr = $.post(baseUrl + '/categories/delete', {deleteCategoriesQueue: deleteCategoriesQueue, csrf: PassHub.csrf}, function() { 
          ...
          .fail(function() {            
            if(jqxhr.responseText != '') {
              ...
              PassHub.error = 'Error: failed to delete categories';
            }
      ...
      // Submit new/updated field data with AJAX
      // returns loginId
      var jqxhr = $.post(baseUrl + '/categories/save/', {categoryData: fieldData, csrf: PassHub.csrf}, function(data) { 
        Materialize.toast('Categories saved', 4000);
      })
        .always(function() {
          // Save button return to normal state
          setButtonLoadingIndicator(false, saveButtonEl);
        ...
        .fail(function() {
          if(jqxhr.responseText != '') {
            ...
            PassHub.error = 'Error: failed to update categories';
          }
          Materialize.toast(PassHub.error, 4000);
        });
  ...
  if( PassHub.mode == 'groups' ) {
    ...
    getItemJSON('groups', PassHub.csrf, function(data) {
      ...
      PassHub.categories  = data.categories;
      ...
      switch(action) {
        ...
        case 'deleteGroup':
          ...
          if(itemIndex <= 2 && isNaN(itemIndex) === false) {
            Materialize.toast('You can not delete the admin nor user group.', 4000);
            break;
          }
          // Confirm action if the group exists in the database (has an ID/index)
          if(isNaN(itemIndex) === false) {
            if( ! confirm('You are about to delete this group. Associated users will need to be re-assigned to a new group on the "Users" page. Go ahead anyway?')) {
              break;
            }
            // Instruct user what to do to finalize the deletion and cleanup affected users.
            var groupName = $('.name-field', fieldEl).val();
            Materialize.toast('The "' + groupName + '" group will be deleted if you hit save. You may re-assign users to a group on the Users page.', 8000);  
    ...
    $('body').on('click', '.btnSave', function(e) {
      ...
      $('.input-field', cardContentInnerEl).each(function() {
        ...
        if(itemIndex == 'undefined' || itemIndex == '0' || itemIndex == '') {
          saveData.groups.add.push({'name': name });
        }
      });
      ...
      var jqxhr = $.post(baseUrl + '/groups/save/', {saveData: saveData, csrf: PassHub.csrf}, function(data) { 
        Materialize.toast('Groups saved', 4000);
      })
        .always(function() {
        ...
        .fail(function() {
          var error = '';
            if(jqxhr.responseText != '') {
              ...
              error = 'Error: failed to update groups';
            }
          Materialize.toast(error, 4000);
        });
        ...
        var jqxhr2 = $.post(baseUrl + '/groups/delete', {deleteItemsQueue: deleteItemsQueue, csrf: PassHub.csrf}, function() { 
          ...
          .fail(function() {
            ...
            if(jqxhr2.responseText != '') {
              error2 = jqxhr2.responseText;
            } else {
              error2 = 'Error: failed to delete groups';
            }
  ...
  if( 
    ...
    getItemJSON('users', PassHub.csrf, function(data) {
      ...
      if( PassHub.mode == 'edit-account') {
        ...
        $('.delete-form p').first().text('Delete your account permanently?');
      }
    });
    ...
    $('body').on('click', '.itemSingle .actions button', function(e) {
      ...
      switch(action) {
        case 'generatePassword':
          ...
          Materialize.toast('Make sure to store the password in a safe place before saving!', 4000);
          break; 
      }
    });
    ...
    $('body').on('click', '.itemSingle .btnSave', function(e) {
      ...
      var jqxhr = $.post(baseUrl + endpoint, {itemData: itemData, csrf: PassHub.csrf}, function(data) { 
        ...
        if(PassHub.mode == 'users') {
          ...
          Materialize.toast('Account saved', 4000);
        }
      })
        .always(function() {
          // Save button return to normal state
